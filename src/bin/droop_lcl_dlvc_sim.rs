...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    */
    ...
    let w_cur = 2.*PI*3000.;  // TODO: Determine why these frequencies needed to be cranked up this high for tracking. Guessing that per-unitization is the underlying factor
    let w_vol = 2.*PI*2000.;   // Originally was using 2.*PI*5000 and 2.*PI*800, but found 1e6 and 3e5 work well (Seperation is a bit low though <10x). Possibly multiply by w_nom, so remove 1/w_nom below?
    ...
    let mut line_to_bus: LineToBus<f32, 8, 6> = build_line_to_bus(&mut line, bus);  // TODO: Determine if these references need to be mutable and try to make it so that LTB can build its own components
    ...
    */
    ...
    let t_step = t_end * 0.5; // Active power reference step time
    let t_switch = t_end * 0.2;  // Grid-side switch time
    ...
    for step in steps {
        ...
        delta = vc_theta / w_nom - line_to_bus.x[(1)];
        ...
        // Sample the current
        i_alpha_sample = line_to_bus.x[(6)];
        i_beta_sample = line_to_bus.x[(7)];
        // Sample the grid
        ...
        v_inv_dq = voltage_loop.step_output(dt, [v_gfm[0] * SQRT_2, dv_dt_gfm[(1)], vc_dq.d, vc_dq.q, if_dq.d, if_dq.q, ig_dq.d, ig_dq.q]);  // TODO: Should w_gfm by in per-unit or rad/s?
    }
