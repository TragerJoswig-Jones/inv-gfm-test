...
const VOLTAGE_FILE_NAME: &'static str = "images/droop_rl_dc_sim_voltage.png";
const THETA_FILE_NAME: &'static str = "images/droop_rl_dc_sim_thetas.png";
const POWER_OUT_FILE_NAME: &'static str = "images/droop_rl_dc_sim_powers.png";
const CURRENT_OUT_FILE_NAME: &'static str = "images/droop_rl_dc_sim_currents.png";
const DELTA_OUT_FILE_NAME: &'static str = "images/droop_rl_dc_sim_deltas.png";
const V_DC_FILE_NAME: &'static str = "images/droop_rl_dc_sim_vdc.png";
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    */
    ...
    let v_dc_nom = 200.;  // dc-side nominal voltage
    ...
    // DC PI controller parameters
    let kp_dc: f32 = 1.0;  // TODO: Tune these parameters
    let ki_dc: f32 = 10.;
    ...
    let mut capacitor_volt_ctrl = PiController::new(kp_dc, ki_dc, 1.);
    let mut capacitor: Capacitor<f32> = Capacitor::new(v_nom, rc, c);
    let mut dc_p_input: f32 = 0.;  // Start at 0 W during presynchronization
    ...
    */
    ...
    let t_step = t_end * 0.5; // DC active power input step time
    ...
    let mut i_c: f32; let mut v_dc: f32 = 1.;
    ...
    let mut v_dc_values: Vec<(f32, f32)> = vec![(0., 0.); (n_steps+1) as usize];
    let mut p_ref_values: Vec<(f32, f32)> = vec![(0., 0.); ((n_steps+1)*cont_n_steps) as usize];
    ...
    let mut v_dc_values: Vec<(f32, f32)> = vec![(0., 0.); ((n_steps+1)*cont_n_steps) as usize];
    ...
    for step in steps {
        ...
        // Sample the dc-side voltage
        i_c = (dc_p_input - p) / v_dc;
        let v_dc_sample = capacitor.get_voltage([i_c])[0];
        // Get alpha-beta gfm voltage
        let mut v_inv_alpha_beta = AlphaBeta::from_polar(v_inv[0], v_inv[1] * gfm.ctrl.get_w_nom());

        // Step the system
        for n in 0..cont_n_steps {
            // Calculate power
            v_inv_alpha_beta.alpha *= v_dc / v_dc_sample;  // v_inv is scaled by v_dc
            v_inv_alpha_beta.beta *= v_dc / v_dc_sample; 
            ...
            p_ref_values[(cont_n_steps*step + n) as usize] = (t + (n as f32)*cont_dt, p_ref);  // save each iteration for plotting
            
            // Calculate dc-side current (Assuming lossless inverter)
            i_c = (dc_p_input - p) / v_dc;

            // Step the dc-side of the system
            capacitor.step(cont_dt, [i_c]);
            v_dc = capacitor.get_voltage([i_c])[0];
            v_dc_values[(cont_n_steps*step + n) as usize] = (t + (n as f32)*cont_dt, v_dc);

            // Step the ac-side of the system
        ...
        // Step the controllers after a z^-1 delay
        capacitor_volt_ctrl.step(dt, [v_dc_sample]);

        p_ref = capacitor_volt_ctrl.get_output();
        gfm.set_p_ref(p_ref);
    ...
    */
    ...
    let v_dc_values_ = v_dc_values.to_vec();
    let p_ref_values_ = p_ref_values.to_vec();
    ...
    let (_,mut prefs): (Vec<_>, Vec<_>) = p_ref_values.into_iter().unzip();
    ps.append(&mut qs);
    ps.append(&mut prefs);
    ...
    /* Plot the dc-side voltage data */
    let (_,mut vs): (Vec<f32>, Vec<f32>) = v_dc_values.into_iter().unzip();
    let min_v: f32 = vs.iter().fold(0.0f32, |a, &b| a.min(b));
    let max_v: f32 = vs.iter().fold(0.0f32, |a, &b| a.max(b));
    let root = BitMapBackend::new(V_DC_FILE_NAME, (640, 480)).into_drawing_area();
    root.fill(&WHITE)?;
    let mut chart = ChartBuilder::on(&root)
        .caption("DC Voltage", ("sans-serif", 50).into_font())
        .margin(5 as u32)
        .x_label_area_size(30 as u32)
        .y_label_area_size(30 as u32)
        .build_cartesian_2d(0.0f32..t_end, (min_v-1.)..(max_v+1.))?;
