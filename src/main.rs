...
const VOLTAGE_FILE_NAME: &'static str = "images/voltage.png";
const THETA_FILE_NAME: &'static str = "images/thetas.png";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    */
    ...
    let xi: f32 = 15.;
    let c: f32 = 0.2679;
    let mut inv = dvoc_controller(v_nom, w_nom, s_rated, xi, c);
    inv.theta = -dt * 0.475 * w_nom;
    inv.v = v_nom * 0.999965;
    ...
    // ODE solver settings
    let rtol = 1.0e-4; let atol = 1.0e-4;
    let ode_steps = 20.;
    ...
    */
    ...
    'sim: for step in steps {  // TODO: Debug this simulation. Power / Current values seem wonky. Something to do with the per unitization of theta
        ...
        let v = alpha_beta_fr_polar(inv.v, inv.theta);
        ...
        let mut ode_inv = dvoc_controller(v_nom, w_nom, s_rated, xi, c);
        ...
        match res {
            Ok(_stats) => {
                ...
                let k = ys.len() - 1;
                bus.v = ys[k][(0, 0)] as f32; bus.theta = ys[k][(1, 0)] as f32 % (2.*PI);
...
impl ode_solvers::System<State> for DigitalInverterSMIB {
    fn system(&self, _t: f64, y: &State, dy: &mut State) {
        ...
        let (di_alpha_dt, di_beta_dt) = self.line.dynamics([y[2], y[3]], [self.inv.v, self.inv.theta, 
                                                                                         y[0], y[1]]);
        dy[0] = dvg_dt;
        dy[1] = dthetag_dt;
        dy[2] = di_alpha_dt;
